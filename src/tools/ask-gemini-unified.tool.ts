...
export const askGeminiUnifiedTool: UnifiedTool = {
  name: "ask-gemini",
  ...
  execute: async (args, onProgress) => {
    ...
    try {
      ...
      if (!numChatId || numChatId === 0) {
        ...
        const chat = await chatManager.getChat(numChatId, agentName);
        if (!chat) {
      ...
      const addResult = await chatManager.addMessage(targetChatId, agentName, prompt);
      if (!addResult.success) {
        return addResult.message;
      }

      // Get updated chat for context
      const chat = await chatManager.getChat(targetChatId, agentName);
      if (!chat) {
